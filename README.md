...
## 概要
  ...
  * インターバル設定をすることで自動的に設定した秒数間隔(1秒~6か月程度)で自動的にインターバル記録できます  
...
## 仕様
  ...
  * PCを介したシリアルコンソール上から変更できるパラメータ:  
  記録インターバル、ファイル形式、画像解像度、日付時刻、代替インタフェース設定、データフレームインターバル
...
## 注意点
  * マイコン上で処理する都合上、完全なUVC規格でないため、カメラによって動作しないor不安定な場合があります  
  ...
  * ボタンやコマンドから取得命令後、ストリーミングデータから切り出し処理をして保存されるため、タイムラグがあります  
  * BMP形式の場合、メモリの制約によって最大解像度は320x240です  
  * JPG形式の場合、最大解像度上限はありませんが解像度によって動作、非動作の場合があります  
  * 電源不安定な場合や電流が足りない場合、動作確認済のカメラであっても正常に動作しない場合があります  
...
## 応用編
  ...
  * LCDのモニタはJPGの場合、TJPGD3ライブラリでデコードしてから描画します(解像度幅160、320、640、1280のみ対応)  
